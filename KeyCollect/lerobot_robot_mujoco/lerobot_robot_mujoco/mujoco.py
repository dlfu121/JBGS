...
class MuJoCoRobot(Robot):
    ...
    def get_observation(self, camera_names: list[str] | None = None) -> dict[str, Any]:
        """Get current robot state + camera images.

        Args:
            camera_names: Optional camera feature names to render. Names ending
                in ``_depth`` request metric depth. When omitted, render every
                configured RGB and depth feature as required by LeRobot's
                recording interface.
        ...
        requested_cameras = set(camera_names) if camera_names is not None else None
        if requested_cameras is not None:
            available_cameras = set(self._camera_configs)
            available_cameras.update(
                f"{name}_depth" for name in self.config.depth_camera_names
            )
            unknown_cameras = requested_cameras - available_cameras
            if unknown_cameras:
                raise ValueError(
                    "Unknown camera observation(s): "
                    f"{sorted(unknown_cameras)}; available: {sorted(available_cameras)}"
                )
        ...
        for cam_name in self.config.depth_camera_names:
            ...
            if requested_cameras is not None and depth_name not in requested_cameras:
                continue
            w, h = self._camera_configs[cam_name]
            obs[depth_name] = self._sim.render_depth_camera(
