...
def is_hardware_opengl_renderer(renderer: str | None) -> bool:
    """Return whether an OpenGL renderer string identifies hardware rendering."""
    if not renderer:
        return False
    normalized = renderer.casefold()
    return not any(marker in normalized for marker in _SOFTWARE_RENDERER_MARKERS)
...
class MuJoCoSimulation:
    ...
    def get_opengl_info(self) -> dict[str, str | bool | None]:
        """Report the actual OpenGL context used by ``mujoco.Renderer``."""
        if self._renderer is None:
            raise RuntimeError("Render a camera before querying OpenGL information.")

        context = getattr(self._renderer, "_gl_context", None)
        ...
        try:
            ...
            raise RuntimeError(
                "PyOpenGL is required to verify the active OpenGL renderer."
            ) from exc
        ...
        return {
            ...
            "renderer": renderer,
            "version": version,
            "hardware_accelerated": is_hardware_opengl_renderer(renderer),
        }
