...
python scripts/train_act_depth.py \
  ...
  --dataset.root=/home/ee304/jbgs/mujoco-3.10.0/model.test/worker_scene/datecollect_vla/data/rm65_dexhand_depth_merged \
...
MuJoCo renderer: backend=egl, vendor=NVIDIA Corporation, renderer=..., hardware=True
Controlling ['table_camera', 'wrist_overhead_camera', 'table_camera_depth'] cameras at 30 Hz
```

`--device cuda` 会在 CUDA 不可用时直接报错，避免无意中退回 CPU；需要自动选择时可改为
`--device auto`。

`--render-backend egl --headless --require-gpu-rendering` 会使用无头 OpenGL 渲染相机并
检查实际 `GL_RENDERER`；如果落到 `llvmpipe`、`softpipe` 等 CPU 软件渲染器会直接报错。
它与 ACT 的 `--device cuda` 是两套独立设备选择。需要显示交互窗口时改用
`--render-backend glfw` 并删除 `--headless`；GLFW 是否使用 GPU 同样以启动日志中的
`MuJoCo renderer` 为准。

保留交互窗口并强制硬件 OpenGL 的双 RGB 推理命令为：

```bash
MUJOCO_GL=glfw python scripts/infer_mujoco.py \
  --checkpoint outputs/train/act_rm65_dexhand \
  --dataset-root /home/ee304/jbgs/mujoco-3.10.0/model.test/worker_scene/datecollect_vla/data/rm65_dexhand_merged \
  --device cuda \
  --render-backend glfw \
  --require-gpu-rendering \
  --random-seed 42
...
- `config/record_mujoco.yaml`：机器人、动捕、相机和数据集参数。
...
| 参数 | 说明 |
|---|---|
| `--profile depth` / `rgb` | 选择深度或旧 RGB 采集/合并 profile；`depth` 为默认 |
| `--dry-run` | 预览采集编号和实际命令 |
| `--dataset.num_episodes` | 临时覆盖 episode 数量 |
| `--dataset.episode_time_s` | 临时覆盖单条时长 |
| `--data_root` | 合并脚本的采集数据根目录（如 `/media/ee304/FDL`） |
| `--exclude N ...` | 合并时跳过指定采集批次 |
| `--dataset.root` | 数据集目录（深度/旧 RGB 训练与推理均需与 checkpoint 对应） |
| `--dataset.depth_output_unit=m` | 深度输出单位，必须为米 |
| `--dataset.video_backend=pyav` | 视频解码后端 |
| `--device cuda\|cpu\|auto` | 推理设备 |
| `--render-backend egl\|glfw` | 渲染后端 |
| `--headless` | 无头渲染 |
| `--require-gpu-rendering` | 强制要求实际使用 GPU 渲染器 |
| `--random-seed` | 随机种子 |

### 环境常量

- Linux 用户：`ee304`
- KeyCollect：`/home/ee304/jbgs/mujoco-3.10.0/model.test/worker_scene/datecollect_vla`
- Conda 环境：`keycollect`
- ROS：ROS1 Noetic
...
- 深度帧以浮点米为输入，由 LeRobot 的 depth encoder 单独编码；它不会被当作普通 RGB 视频。
- 不要向旧的 RGB-only 数据集 resume 写入，因为新旧 feature schema 不同。
- 合并脚本按 profile 筛选，因此不会把旧 RGB 数据误混进深度数据。
