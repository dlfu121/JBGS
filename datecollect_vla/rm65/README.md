...
| 文件 | 用途 |
|---|---|
| `rm65_ik.py` | RM65 几何封闭解析 IK 求解器与连续选择器 |
| `test_rm65_ik.py` | RM65 运动学自检/单元测试 |
| `matlab_validation/` | MATLAB 封闭解析解验证工程（见 `matlab_validation/README_MATLAB.md`） |
...
- `sample_period`：采样周期（示例 `1 / 30`）。
- `initial_seed`：第一帧用于确定起始解析支路的当前关节角。
- `selector.reset(current_joints, degrees=True)`：每条新轨迹开始前重置。
- `selector.last_cost`、`selector.last_cost_terms`：查看最近一次选择的总代价和分项。

它会惩罚肩部、肘部和腕部分支跳变、速度超限、速度突变、逼近关节限位以及腕部奇异附近 J4/J6 的大幅补偿运动。

## 备注

`inverse_all()` 枚举关节限位内的肩部、肘部和腕部解析分支。`inverse()` 根据
`seed` 返回最近的一组，实际连续轨迹中应传入机械臂当前关节角。腕部奇异时存在
无穷多解，程序使用 `seed` 中的 J4 固定一组代表解。目标矩阵描述的是法兰坐标系；
若使用工具中心点（TCP），应先右乘工具变换的逆矩阵得到法兰目标。

部署到真机前，请用示教器读取若干已知关节角/末端位姿进行坐标系和型号复核，并在低速模式下验证。

MATLAB 封闭解析解、30 Hz 机械臂动画、六关节角/角速度监视和批量验证位于
`matlab_validation` 文件夹。直接运行 `matlab_validation/run_rm65_validation.m`，
详细说明见 `matlab_validation/README_MATLAB.md`。
