...
| 参数 | 说明 |
|---|---|
| 机器人型号 | `rm65_parameters("RM65-B")`，可选 RM65-B、RM65-B-V、RM65-6FB、RM65-6FB-V、RM65-6F |
| 动画时长 | `rm65_validate_animation` 第二个参数，单位秒（默认 30 Hz） |
| 实时播放 | `rm65_validate_animation` 第三个参数，`true` 按墙钟播放，`false` 快速跑完 |
| 批量样本数 | `rm65_batch_test("RM65-B", 500)` 第二个参数 |
| `d6` | 由型号决定，B=144 mm、6FB=161 mm、6F=172.5 mm |

## 备注

网页抓取结果没有完整显示型号表中的 `d6` 数值，因此当前按照官方工作半径与
`256 + 210 + d6` 的几何关系设置为：B=144 mm、6FB=161 mm、6F=172.5 mm。真机
验证前请以具体机械臂铭牌和厂家原始参数表复核 `d6`。

如果输入的是 TCP 位姿而不是机器人法兰位姿，应先去除工具变换；连续轨迹应把上一点
关节角作为下一点的 `seed`。
