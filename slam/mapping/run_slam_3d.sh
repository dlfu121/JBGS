...
#
# 用法:
#   ./slam/mapping/run_slam_3d.sh              # 自动巡视 + rtabmap + rviz
#   ./slam/mapping/run_slam_3d.sh --view       # 同时开 MuJoCo 查看器
#   ./slam/mapping/run_slam_3d.sh --explore    # Frontier 自主探索建图（无需航点）
#   ./slam/mapping/run_slam_3d.sh --fast       # 2 倍仿真速度，缩短实验墙钟时间
#   ./slam/mapping/run_slam_3d.sh --no-rviz    # 不开 rviz
#   ./slam/mapping/run_slam_3d.sh --fresh      # 清理遗留进程后启动
#   ./slam/mapping/run_slam_3d.sh --scene ariac      # 用 ARIAC 场景（默认）
set -eo pipefail
...
if [ ! -f model/robot/robot_template_3d_py38.xml ]; then
  echo "缺 model/robot/robot_template_3d_py38.xml，请先准备 3D 机器人模型" >&2
  exit 1
fi
...
for a in "$@"; do
  case "$a" in
    --view)     WANT_VIEW=1; BRIDGE_ARGS+=(--view) ;;
    --no-rviz)  WANT_RVIZ=0 ;;
    --fresh)    FRESH=1 ;;
    --teleop)   WANT_PATROL=0 ;;
    --explore)  EXPLORE=1; WANT_PATROL=0 ;;
    --fast)     BRIDGE_ARGS+=(--sim-speed 2.0) ;;
    --scene)    : ;;  # 下面处理
    ariac) SCENE="$a" ;;
    *)          BRIDGE_ARGS+=("$a") ;;
  esac
...
if [ "$SCENE" = "ariac" ]; then
  ...
  python3.8 model/robot/gen_ariac_robot.py \
    --start-x 4.0 --start-y 4.6 --start-z -0.08
else
  echo "未知场景: $SCENE（可选: ariac）" >&2; exit 1
fi
...
# Viewer/GLFW can terminate the bridge immediately when DISPLAY or OpenGL is
# unavailable.  Never continue with an apparently healthy RTAB-Map that has no
# point-cloud producer.  For --view, retry the same bridge headlessly; without
# --view, fail fast because there is no recoverable GUI-specific cause.
if ! kill -0 "$BRIDGE_PID" 2>/dev/null; then
  wait "$BRIDGE_PID" 2>/dev/null || true
  if [ "$WANT_VIEW" = 1 ]; then
    echo "  ★ MuJoCo 查看器启动失败，自动回退到无界面建图模式" >&2
    HEADLESS_ARGS=()
    for arg in "${BRIDGE_ARGS[@]}"; do
      [ "$arg" = "--view" ] || HEADLESS_ARGS+=("$arg")
    done
    python3.8 "$BRIDGE_SCRIPT" "${HEADLESS_ARGS[@]}" &
    BRIDGE_PID="$!"
    PIDS+=("$BRIDGE_PID")
    sleep 5
  fi
...
if ! kill -0 "$BRIDGE_PID" 2>/dev/null; then
  echo "MuJoCo 3D 点云桥接启动失败；停止启动 RTAB-Map。" >&2
  exit 1
fi

echo "[2/4] 起 MapCloud 地面点过滤 ..."
python3.8 "$HERE/slam/mapping/cloud_map_height_filter.py" --ros-args \
  -p min_height:=0.18 &
PIDS+=("$!")
sleep 1

echo "[3/4] 起 rtabmap 3D SLAM ..."
ros2 launch "$HERE/slam/mapping/rtabmap_3d.launch.py" use_sim_time:=true &
PIDS+=("$!")
sleep 4

if [ "$WANT_RVIZ" = 1 ]; then
  echo "[4/4] 起 RViz (slam/slam_3d.rviz) ..."
  rviz2 -d "$HERE/slam/mapping/slam_3d.rviz" --ros-args -p use_sim_time:=true &
  PIDS+=("$!")
fi
...
if [ "$EXPLORE" = 1 ]; then
  ...
  python3.8 "$HERE/slam/mapping/frontier_explorer.py" &
  PIDS+=("$!")
fi
...
保存地图（建图完成后执行）：
  ...
  # 导出 2D 投影栅格（nav2 可用）
  ros2 run nav2_map_server map_saver_cli -f ariac_map_3d --ros-args -p use_sim_time:=true
