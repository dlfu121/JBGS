#!/usr/bin/env bash
# 只重启导航节点 slam/navigation/nav_p2p.py，MuJoCo 仿真/RViz 窗口保持不动。
# 用于"边改代码边看效果"：改完 slam/navigation/nav_p2p.py 跑一次本脚本即可。
set -e
...
python3.8 slam/navigation/nav_p2p.py --use-saved > /tmp/opencode/nav_p2p.log 2>&1 &
sleep 2
