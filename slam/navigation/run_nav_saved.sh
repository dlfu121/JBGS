...
#
# 启动内容：
#   1. bridge_ariac.py  — MuJoCo 仿真 + TF + /clock + 实时点云
#   2. slam/navigation/nav_p2p.py --use-saved  — 保存地图全局规划 + 条件式动态避障
#   3. RViz2  — 可视化地图和路径，点击目标点导航
#
# 用法:
#   ./slam/navigation/run_nav_saved.sh              # ARIAC, headless
#   ./slam/navigation/run_nav_saved.sh --view       # 同时开 MuJoCo 查看器
#   ./slam/navigation/run_nav_saved.sh --scene ariac --task inspect --view
#   ./slam/navigation/run_nav_saved.sh --scene ariac --task vla --view
#   ./slam/navigation/run_nav_saved.sh --dynamic-person  # 动态行人测试（ARIAC）
#
# 在 RViz2 中：
...
echo "[2/3] 起导航节点 (slam/navigation/nav_p2p.py --use-saved --scene $SCENE) ..."
python3.8 slam/navigation/nav_p2p.py --use-saved --scene "$SCENE" &
PIDS+=("$!")
sleep 3
if ! kill -0 "${PIDS[2]}" 2>/dev/null; then
  echo "错误：slam/navigation/nav_p2p.py 启动后立即退出；请检查地图路径或 Python/ROS 环境。" >&2
  exit 1
fi

echo "[3/3] 起 RViz2 ..."
rviz2 -d "$HERE/slam/navigation/view_map.rviz" --ros-args -p use_sim_time:=true &
PIDS+=("$!")
