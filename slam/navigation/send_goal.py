...
坐标系说明:
  ...
  --frame world 世界系（自动做 world -> map 换算；ARIAC 起点 world(4, 4.6)=map 原点）

用法:
  python3.8 slam/navigation/send_goal.py 3.0 2.0                 # map 系 (3.0, 2.0)
  python3.8 slam/navigation/send_goal.py --world -4 1            # 世界系 (-4, 1) = map (4.5, 1)
  python3.8 slam/navigation/send_goal.py --frame map 2.0 -1.5 --wait   # 发完并等待到达/状态
"""
import argparse
import math
import sys
...
class GoalSender(Node):
    def __init__(self, x, y, frame, wait, scene, yaw_deg):
        ...
        if frame == "world":
            ...
            gy = y - origin[1]
        self.goal = (gx, gy)
        self.yaw = None if yaw_deg is None else math.radians(yaw_deg)
