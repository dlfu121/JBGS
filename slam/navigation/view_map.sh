...
# 用法:
#   ./slam/navigation/view_map.sh                          # 默认打开 ARIAC 3D 地图
#   ./slam/navigation/view_map.sh maps/ariac/ariac_map_3d.yaml # 指定地图文件
set -eo pipefail
...
MAP_YAML="${1:-$HERE/maps/ariac/ariac_map_3d.yaml}"
if [ ! -f "$MAP_YAML" ]; then
...
rviz2 -d "$HERE/slam/navigation/view_map.rviz" --ros-args -p use_sim_time:=false &
PIDS+=("$!")
